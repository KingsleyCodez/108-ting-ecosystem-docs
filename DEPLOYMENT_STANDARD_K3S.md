...
4. Deploy through Helm (never around it): run `helm upgrade <rel> "$CHART_PATH" -n <ns> --reset-then-reuse-values --set image.repository="ghcr.io/108-plaza/<svc>" --set image.tag="sha-<short>" --wait --timeout 3m`, followed by `kubectl rollout status` and verifying `readyReplicas >= 1` (see [HELM_DEPLOY_PORTING_GUIDE.md](HELM_DEPLOY_PORTING_GUIDE.md)). Passing both repo and tag on every run ensures the release self-corrects and prevents drift.
